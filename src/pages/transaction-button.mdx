...
A transaction-submission preset over [MultiStateButton](/multi-state-button).
Ships a seven-state lifecycle with default content and a colour palette per
state, so the common case is zero-config. Everything stays overridable per
state.
