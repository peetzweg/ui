...
Set `loop` to wrap around at the ends instead of clamping — step past the last
frame and it returns to the first, and vice-versa. This stack is `scroll`-driven
and uncontrolled: scroll over it to scrub (arrow keys work too), and it never
hits a dead end.
