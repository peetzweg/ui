...
import { TimeMachineScrollDemo } from '@/registry/new-york/examples/time-machine-scroll-demo'
import { TimeMachineLoopDemo } from '@/registry/new-york/examples/time-machine-loop-demo'
...
Add `loop` and the same scroll stack wraps instead of clamping — scrub past the
last frame and it returns to the first, with no dead ends:
